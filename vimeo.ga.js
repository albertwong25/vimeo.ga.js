...
(function () {
  ...
  function fireEvent(action, id, url, title, duration, value) {
    if (id == null || title == null || duration == 0) {
      return
    }
    var category = "Vimeo Video" + " | " + url;
  ...
  function trackPlayer(iframe) {
    ...
    player.getVideoId().then(function (v) {
      id = v;
      fireEvent("load", id, url, title, duration, value)
    });
    ...
    player.on("play", function (data) {
      value = data.seconds;
      fireEvent("play", id, url, title, duration, value);
      player.off("play")
    });
    player.on("pause", function (data) {
      value = data.seconds;
      fireEvent("pause", id, url, title, duration, value)
    });
    player.on("ended", function (data) {
      value = data.seconds;
      fireEvent("progress - 100%", id, url, title, duration, value)
    });
    player.on("emailcapture", function (data) {
      value = data.seconds;
      fireEvent("emailcapture", id, url, title, duration, value)
    });
    player.on("timeupdate", function (data) {
      ...
      if (prev_progress !== progress) {
        fireEvent("progress - " + progress * 100 + "%", id, url, title, duration, value)
      }
    });
    window.onbeforeunload = function (e) {
      fireEvent("leave", id, url, title, duration, time) /*player.getCurrentTime().then(function(v){value=v;fireEvent("leave",id,title,duration,value)})*/
    }
  }
